...
<%
  ...
  Htmpl htmpl = HtmplFactory.getInstance(application)
                ...
                .getHtmpl("/anag/layout/dettaglioAziendaSoggetti.htm");
%><%@include file = "/include/menu.inc" %><%
  ...
  htmpl.set("dittaUMA", "Ditta N. "+dittaVO.getDittaUMAstr()+" / "+dittaVO.getTipiDitta());



  if(soggettiVector!=null){

    for(int i=0;i<soggettiVector.size();i++){

      PersonaFisicaVO pfVO = (PersonaFisicaVO)soggettiVector.elementAt(i);

      htmpl.newBlock("rigaSoggetti");

      htmpl.set("rigaSoggetti.ruolo",pfVO.getRuolo());

      htmpl.set("rigaSoggetti.cognome",pfVO.getCognome());

      htmpl.set("rigaSoggetti.nome",pfVO.getNome());

      htmpl.set("rigaSoggetti.codiceFiscale",pfVO.getCodiceFiscale());

      htmpl.set("rigaSoggetti.dataInizioRuolo",DateUtils.formatDate(pfVO.getDataInizioRuolo()));
