...
<%
    SolmrLogger.debug(this,"   BEGIN datiLavorazioniCpPOPView");

    String layoutUrl = "/domass/layout/datiLavorazioniCpPOP.htm";
    ValidationException valEx;
    Validator validator = new Validator(layoutUrl);
    Htmpl htmpl = HtmplFactory.getInstance(application).getHtmpl(layoutUrl);

    SolmrLogger.debug(this, "--- recupero l'elenco delle Lavorazioni Conto Proprio");
    Vector<LavContoProprioVO> elencoLavContoProprio = ( Vector<LavContoProprioVO>)request.getAttribute("elencoLavorazioniPop");
    ...
    if(elencoLavContoProprio == null || elencoLavContoProprio.size() == 0){
      ...
      Vector<CarburanteFrammentazioneVO> elencoCarburantiPerFrammentaz = (Vector<CarburanteFrammentazioneVO>)request.getAttribute("elencoCarburantiPerFrammentaz");
