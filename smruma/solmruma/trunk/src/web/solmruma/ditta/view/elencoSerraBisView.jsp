...
<%
  ...
  Htmpl htmpl = HtmplFactory.getInstance(application).getHtmpl("ditta/layout/elencoSerreBis.htm");
%><%@include file = "/include/menu.inc" %><%
  ...
  for(int i=startRow;i<size && i< startRow+SolmrConstants.NUM_MAX_ROWS_PAG;i++)
  {
    ...
    if(serraVO.getDataScarico() != null)
      htmpl.set("blkElencoSerre.dataScarico",DateUtils.formatDate(serraVO.getDataScarico()));
    ...
    htmpl.set("blkElencoSerre.dataInizioValidita",DateUtils.formatDate(serraVO.getDataInizioValidita()));
    htmpl.set("blkElencoSerre.linkIdSerra",serraVO.getIdSerra().toString());

    if(serraVO.getDataFineValidita() != null)
      htmpl.set("blkElencoSerre.dataFineValidita",DateUtils.formatDate(serraVO.getDataFineValidita()));
  }
