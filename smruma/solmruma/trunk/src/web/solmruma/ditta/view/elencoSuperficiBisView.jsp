...
<%
  ...
  Htmpl htmpl = HtmplFactory.getInstance(application).getHtmpl("ditta/layout/elencoSuperficiBIS.htm");
%>
...
<%
  ...
  if (superfici==null)
  {
    superfici=new Vector<SuperficieAziendaVO>(); // Evito nullpointerexception
  }

  SolmrLogger.debug(this,"superfici.size()="+superfici.size());
  ...
  for(int i=startRow;i<size && i< startRow+SolmrConstants.NUM_MAX_ROWS_PAG;i++)
  {
    ...
    htmpl.set("blkSuperficie.dataInizioValidita",formatDate(superficieVO.getDataInizioValidita()));
    htmpl.set("blkSuperficie.dataFineValidita",formatDate(superficieVO.getDataFineValidita()));
  ...
  {
    SuperficieAziendaVO superficieVO=(SuperficieAziendaVO)superfici.get(i);
    if (superficieVO.getDataFineValidita() == null)
    {
      supUtilizzataDouble = superficieVO.getSuperficieUtilizzataDouble();
