...
<%
  ...
  Htmpl htmpl = HtmplFactory.getInstance(application).getHtmpl("ditta/layout/elencoLavConsorzi.htm");
%><%@include file = "/include/menu.inc" %><%
  ...
  SolmrLogger.debug(this,"NELLA VIEW elencoLavConsorzi vale: "+elencoLavConsorzi);
  if (elencoLavConsorzi==null)
  {
    elencoLavConsorzi=new Vector();
  }
  SolmrLogger.debug(this,"NELLA VIEW elencoLavConsorzi.size() vale: "+elencoLavConsorzi.size());
  ...
  if(elencoLavConsorzi!=null && elencoLavConsorzi.size()>0){
	  ...
	  htmpl.set("blkDati.totaleGasolio",StringUtils.checkNull(totaleGasolio));
  	  htmpl.set("blkDati.totaleBenzina",StringUtils.checkNull(totaleBenzina));
 ...
 // carico combo anniCampagna
  ...
  if(null!=annoCampagnaSel && !StringUtils.isStringEmpty(annoCampagnaSel.getAnnoCampagna()) && elencoLavConsorzi!=null && elencoLavConsorzi.size()>0){
		...
		String verso = (String)session.getAttribute("versoLavorazione");
		htmpl.set("blkSiLavorazioni.verso",(verso.equals(SolmrConstants.VERSO_LAVORAZIONI_E))?"ha effettuato":"farà effettuare");
  }
